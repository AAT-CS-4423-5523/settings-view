...
export default class PackageCard {
  ...
  */
  ...
  detail = "Any package you install will have access to your system with the same permissions Atom does. Do you trust this source?"
    const notification = atom.notifications.addWarning(`${pack.name} Install ${message}`, {
    dismissable: true,
    detail,
    buttons,
  })
}
    // make a new warning panel based on the package
    // var warningPanel = new WarningPanel(this.pack)
    // options =
    // atom.notifications.addWarning("this is a test")
